"""
Example: update your OkCupid bio using updateEssay.

Usage (from backend/):
    python -m examples.update_bio_example
"""
...
def update_bio_for_client(client: OkCupidClient, new_bio: str) -> Dict:
    """
    Shared helper used by both this example script and the API server to
    update the current user's bio text.
    """
    updated = profile.update_my_bio(client, new_bio)
    return updated
...
def main() -> None:
    ...
    new_bio = (
        os.getenv("NEW_BIO_TEXT")
        or str(profile_cfg.get("bio") or "").strip()
        or "snap; nadii3yz  \nthis app moves slow for me"
    )

    updated = update_bio_for_client(client, new_bio)
    print("=== Updated bio essay ===")
    print(updated.get("rawContent") or updated.get("processedContent"))
